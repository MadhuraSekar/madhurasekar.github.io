...
const T = {
  bg: '#08090d', surface: '#0c0e12',
  border: '#1a1d24', green: '#00e087',
      ...
      <div style={{ width: '100%', maxWidth: 400, padding: 24 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <input type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)}
            style={{ fontFamily: sans, fontSize: 14, padding: '12px 14px', borderRadius: 8, background: T.surface, color: T.text, border: `1px solid ${T.border}`, outline: 'none' }} />
          <input type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)}
            style={{ fontFamily: sans, fontSize: 14, padding: '12px 14px', borderRadius: 8, background: T.surface, color: T.text, border: `1px solid ${T.border}`, outline: 'none' }} />
          {error && <span style={{ fontFamily: sans, fontSize: 12, color: T.red }}>{error}</span>}
          <button type="submit" disabled={loading} style={{
            fontFamily: mono, fontSize: 13, fontWeight: 600, padding: '12px', borderRadius: 8,
        ...
        <div style={{ textAlign: 'center', marginTop: 20 }}>
          <span style={{ fontFamily: sans, fontSize: 12, color: T.muted }}>
            ...
            <a href="/signup" style={{ color: T.green, textDecoration: 'none' }}>Sign up</a>
          </span>
        </div>
